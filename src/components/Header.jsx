...
const NavLink = ({ href, children, active }) => (
  <a
    ...
    className={`${active ? 'text-accent' : 'text-medium-gray hover:text-accent'} transition-colors duration-300 px-3 py-2 text-sm font-medium relative group`}
  >
    {children}
    <span className={`absolute left-0 bottom-0 h-0.5 bg-accent transition-all duration-300 ${active ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
  </a>
);
...
const MobileNavLink = ({ href, children, onClick }) => (
  <a
    href={href}
    onClick={onClick}
    className="block text-light-gray hover:text-accent transition-colors duration-300 px-6 py-4 text-base font-medium border-b border-primary/50"
  >
    {children}
  </a>
);

// Reusable component for social media icons (desktop & mobile)
...
const SECTIONS = ['about', 'experience', 'clients', 'skills', 'projects', 'certifications', 'education'];
...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled || isMenuOpen ? 'glass shadow-lg' : 'bg-transparent'}`}>
      <div className="container  px-6 md:px-12">
        <div className="flex justify-between items-center h-20">

          {/* Logo/Name - Left Section */}
          <a href="#hero" onClick={closeMenu} className="flex items-center text-lg md:text-2xl font-bold text-light-gray hover:text-accent transition-colors duration-300 z-10">
            <img
            ...
            />
            <span>Chaitanya Yelamasetty</span>
          </a>
          ...
          <nav className="hidden lg:flex items-center  space-x-5">
            <ul className="flex space-x-5 ml-10">
              ...
              <li><NavLink href="#skills" active={activeSection === 'skills'}>Skills</NavLink></li>
              <li><NavLink href="#projects" active={activeSection === 'projects'}>Projects</NavLink></li>
              <li><NavLink href="#certifications" active={activeSection === 'certifications'}>Certifications</NavLink></li>
              <li><NavLink href="#education" active={activeSection === 'education'}>Education</NavLink></li>
            </ul>
            ...
            <div className="flex items-center space-x-4 ml-4">
              ...
              {/* RESUME-CTA-DESKTOP — disabled until a fresh PDF is ready; uncomment to re-enable
              <a
                href={`${import.meta.env.BASE_URL}Chaitanya_yelamasetty_Resume.pdf`}
                target="_blank"
                rel="noopener noreferrer"
                className="px-4 py-2 border border-accent text-accent font-semibold rounded-md hover:bg-accent/10 transition-colors shadow-md"
              >
                View Resume
              </a>
              */}
              ...
              <SocialIconLink href="https://www.linkedin.com/in/chaitanya-yelamasetty" label="LinkedIn profile">
      ...
      {/* Mobile Menu - Hidden by default, toggles max-h on click */}
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="lg:hidden overflow-hidden bg-primary/95 border-t border-secondary backdrop-blur-md"
          >
            <nav className="flex flex-col text-center">
              ...
              <MobileNavLink href="#clients" onClick={closeMenu}>Clients</MobileNavLink>
              ...
              {/* RESUME-CTA-MOBILE — disabled until a fresh PDF is ready; uncomment to re-enable
              <div className="px-8 py-6">
                <a
                  ...
                  rel="noopener noreferrer"
                  onClick={closeMenu}
                  className="block w-full text-center px-4 py-3 border border-accent text-accent font-semibold rounded-md hover:bg-accent/10 transition-colors duration-300 shadow-md"
                >
                  Download Resume
                </a>
              </div>
              */}
              <div className="flex justify-center space-x-6 px-6 py-4 border-t border-primary/50">
